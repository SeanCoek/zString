...
Moreover, we implement a static analysis tool and a dynamic profilling type recorder. Our method is more practical in both efficency and accuracy, as supported by the experimental results.
\end{abstract}
...
\begin{Theorem}~\label{thm:tfa}
  In a context-insensitive setting, for all variables $x$ and classes $c$, $c\tflow^*x$ in TFA iff there exists an object abstraction $o$ of $c$ such that $o\in\VPT(x)$ in PTA.
\end{Theorem}
