...
We compare our approach against Class Hierarchy Analysis (CHA) , Variable Type Analysis and context-insensitive points-to analysis. Among these compared approach, we use the hierarchy structure provided in Soot framework, to implemente CHA. VTA and points-to analysis are implemented by Soot team. The reason we do not compare the efficiency against Variable Type Analysis (VTA) is due to its indivisible implementation. The implementation in Soot framework is embeded as a optional subprocess to generated callgraph for points-to analysis. The generated callgraph is accessible by developer. Under this circumstance, we compare our approach against VTA in precision but not efficiency. Total implementation of VTA will be left for our future work. The choice of the context-insensitive points-to analysis is due to our approach also being context-insensitive, thus the results will be comparable. We use iteration algorithm for points-to analysis to calculate a fixpoint for the same reason. A more space-efficient data structure for iteration set is adopted on points-to analysis, which is provided by Soot team. For the reason that points-to analysis will raise $OutOfMemory$ error and crash when we use the same data structure (\ie, $HashSet$) as in our implementation of TFA. In the following tables we use CHA, VTA, PTA and TFA to refer to the results related to class hierarchy analysis, variable type analysis, points-to analysis and type flow analysis, respectively. During the evaluation the following tow research questions are addressed.
\begin{itemize}
...
%	\fullhline
%check & 4.34 & 69.32 & 3.52 & 8718 & 15599 & 10089 & 34406\\
%compiler & 5.39 & 70.34 & 1.14 & 7781 & 11277 & 1990 & 21048\\
%compress & 5.00 & 71.73 & 1.09 & 7428 & 10947 & 1876 & 20251\\
%crypto & 4.43 & 69.22 & 1.13 & 7575 & 11220 & 1846 & 20641\\
%derby & 4.81 & 70.79 & 1.38 & 7979 & 11450 & 2034 & 21463\\
%helloworld & 4.33 & 70.44 & 1.25 & 7230 & 10543 & 1828 & 19601\\
%mpegaudio & 4.35 & 69.13 & 1.04 & 7279 & 10612 & 1830 & 19721\\
%scimark & 4.51 & 71.01 & 1.15 & 7451 & 11344 & 1845 & 20640\\
%serial & 4.53 & 69.20 & 1.07 & 7683 & 10977 & 1886 & 20546\\
%startup & 6.13 & 134.09 & 1.46 & 11597 & 17870 & 2651 & 32118\\
%sunflow & 4.35 & 71.26 & 1.07 & 7245 & 10560 & 1830 & 19635\\
%xml & 5.14 & 140.72 & 1.24 & 7837 & 11673 & 2016 & 21526\\
%	\fullhline
%\end{tabular}
...
For soundness, the recall of VTA, TFA and PTA can not achieve to $1.00$ in most benchmarks except $helloworld$ for TFA. We analyzed those missed records and find out the reason is due to some Java advance features, liked reflection call and JNI call, which we will describe in section~\ref{subsubsec:reflection-call} and section~\ref{subsubsec:jni-call}. In addition, callback mechanism will raise unsoundness under our treatment on reflection, JNI and library call. We will discuss it in section~\ref{subsubsec:callback}. The result shows that VTA and PTA produces lower recall than TFA in general. We study the reason for this gap from reading the source code of Soot and find out there are some different treatments that produce this result. The first one is the reachability analysis used on VTA implemented by Soot team, as well as PTA. Method invoked by reflection, or invoked as a callback function inside reflection, JNI and library, will be denoted as unreachable under this analysis. As a consequence, some methods would not be analyzed by VTA and PTA. Note that even the unreachable method would not be analyzed, a variable receiving its type from JDK would be assigned to possible types based on the preprocessing from Soot. Such that the set of possible reaching types for these variables would not be empty. The second one is that our approach performs conservatively treatment on library. We will discuss it in section~\ref{subsubsec:library}. Moreover, we find out that Soot make some unique treatment on some JDK invocation, which reduce its recall (\eg, for ``$StringBuilder.append()$'', Soot will assume its returning type as $StringBuilder$, instead of leaving it unresolved).
...
%\label{subsubsec:anonymous}
%Anonymous class in Java enables developer to define and instantiate a class at the same time. It is like local class except that it do not have a name. Listing~\ref{lst:anonymous} shows the typical usage of anonymous class. At line 3 a anonymous class is declared and instantiated. Its class name is actually reprensented as ``$A\$1$'' in Java bytecode.
%Hence, variable $x$ is assined to a instance of ``$A\$1$''. We find out that VTA in Soot inappropriately resolves types of $x$ to ``$ThreadLocal$'', instead of ``$A\$1$''. Our implementation of TFA will resovle to ``$A\$1$'' correctly.
%
%\begin{minipage}{\linewidth}
%\vspace{10pt}
%\begin{lstlisting}[caption={Anonymous Class},label={lst:anonymous}]
%class A {
%	void m() {
...
Soot team performs preprocess on some specific library call, for providing a sounder implementation of PTA. We pick up some codes on benchmark $serial$ as shown in Listing~\ref{lst:pta-preprocess}, to describe why the precision of PTA is much lower. At line 5, we invoke a library call ``$method.invoke()$'', which is preprocessed by Soot framework. We put on the difinition of this method in commented lines. The result of this preprocess is that $obj$ will receive its possible types as all the classes this method can return. We check out the number is generally reached to more than $1000$. Since there are almost 20 dynamic records for this case on benchmark $serial$, PTA generated more than $20000$ false positive cases and lower its precision to just $0.014$.
