...
We have implemented a static type analysis tool for context-insensitive type flow analysis, which is written in Java and aims to analyze Java programs. We run our algorithm on the jimple, a typed, 3-address, statemente based intermediate representation provided by Soot framework~\cite{soot}. The implementation has been tested on a collection of benchmark programs from SPECjvm2008~\cite{specjvm}. We conducted several experiments to compare the efficiency and accuracy with CHA and PTA implemented by Soot. In order to further compare accuracy of different approaches, we applied the instrumentation technique to implement a dynamic profiling tool for jimple, which extracts types of called method at runtime. We then conducted a sophisticated comparison on the different static approaches regarding results generated by the dynamic profiler. The experimental result has shown that PTA is in general more efficient than CHA and PTA. In terms of accuracy, PTA is more precise than CHA. Note that due to the comprehensive optimizations for VTA and PTA that are implemented by the Soot team, a decent comparison with VTA and PTA available in Soot is not straightforward. We have tried to create a relatively suitable environment by disabling some of the optimizations in Soot, and eventually reach the conclusion that the precision of TFA is comparable to VTA and PTA on SPECjvm2008. In addition, we confirm that most existing approaches, including PTA and TFA, do not guarantee soundness on all benchmarks, as already discussed in the literature~\cite{LivshitsSSLACGKMV15}. We then look into all the unmatch record to find out the source of imprecision and unsoundness, with a short discussion on the related Java language features such as JDK library calls, Java reflection API, Java Native Interface and the callback mechanism. %%%We will discuss it in the experiment section of this paper. %(Need more citations here, and brief the experimental results.)

Overall the contributions of the paper are stated as follows.
\begin{enumerate}
  \item We have proposed a new relational static semantics called Type Flow Analysis (TFA), which computes a collection of reachable types for variables. Our methodology is enforced by type propagation via value flow between program variables as well field accesses, following predefined syntactical relational rules (C.f.~Definition~\ref{def:base} and Definition~\ref{def:extension}). We have proved equivalence between TFA and the subset based points-to analysis regarding collected types for variables (Theorem~\ref{thm:tfa}), presented in Section~\ref{sec:type-flow-analysis}.
